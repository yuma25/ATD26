...
import { RefreshCcw, Camera, MapPin, Compass, Flag, Award, Clock, History } from 'lucide-react';
...
export default function Home() {
  ...
  useEffect(() => {
    if (isComplete && !completionTime) {
      setCompletionTime(new Date().toLocaleString('ja-JP', {
    ...
    <div className="min-h-screen font-serif selection:bg-[#d4c5a9] text-[#3e2f28] flex flex-col relative">
      ...
      <header className="fixed top-0 left-0 right-0 z-[100] bg-[#e8e2d2]/90 backdrop-blur-md px-4 sm:px-8 py-6 sm:py-10 flex items-center justify-between border-b border-[#3e2f28]/10 shadow-sm">
          <div className="flex flex-col gap-0.5">
            <div className="flex items-center gap-2 sm:gap-3">
              <Compass size={20} className="text-[#3e2f28]/60 flex-shrink-0" strokeWidth={1.5} />
        ...
        <div className="flex items-center gap-4 sm:gap-6 flex-shrink-0">
          <AnimatePresence>
            {syncing && <RefreshCcw size={14} className="animate-spin text-[#3e2f28]/20" />}
          </AnimatePresence>
          <button 
      ...
      <main className="max-w-xl mx-auto px-8 pt-40 sm:pt-56 pb-40 relative flex-1 w-full z-10">
        <div className="relative">
          ...
          <div className="absolute left-[50%] top-0 bottom-0 w-[4px] -translate-x-1/2 overflow-hidden opacity-40">
            ...
            <motion.div 
              initial={{ height: 0 }}
              animate={{ height: `${(acquiredCount / (badges.length || 1)) * 100}%` }}
              transition={{ duration: 1.5, ease: "easeOut" }}
              className="absolute top-0 w-full bg-amber-400 shadow-[0_0_10px_rgba(245,158,11,0.4)] z-10"
              style={{ maxHeight: isComplete ? 'calc(100% - 100px)' : 'calc(100% - 200px)' }}
            />
          </div>
          ...
          <div className="relative z-10 flex flex-col items-center mb-40">
            <div className="relative">
              ...
              <div className="w-24 h-24 bg-[#fdfaf2] border-2 border-[#3e2f28]/60 rounded-full flex flex-col items-center justify-center shadow-lg rotate-[-8deg]">
                <Flag size={28} className="text-[#3e2f28]/80 mb-1" strokeWidth={1.5} />
                <span className="text-[11px] font-black uppercase tracking-[0.2em]">Entry</span>
              </div>
            </div>
          ...
          <div className="relative z-10 flex flex-col items-center mt-60">
            ...
            >
              ...
              {isComplete ? <History size={40} className="relative z-10" /> : <MapPin size={40} strokeWidth={1} className="relative z-10" />}
            </motion.button>
            <div className="mt-8 text-center">
              <p className={`font-black text-[12px] uppercase tracking-[0.6em] ${isComplete ? "text-[#3e2f28]" : "opacity-20"}`}>
            ...
            >
              <div className="relative z-10 space-y-12">
                <div className="border-b-2 border-[#3e2f28] pb-10">
                  <Award size={64} className="mx-auto text-[#3e2f28] mb-6" strokeWidth={1} />
                  <h2 className="text-5xl font-black italic text-[#3e2f28] tracking-tighter">FINAL LOG.</h2>
                </div>

                <div className="space-y-8 py-4 text-[#3e2f28]">
                  <div className="space-y-1">
                    <p className="text-xl font-bold italic">All Specimens Recorded</p>
                  ...
                  <div className="space-y-1">
                    <p className="font-mono text-xs font-black">{completionTime}</p>
                  </div>
      ...
      <footer className="py-12 border-t border-[#3e2f28]/5 bg-black/[0.01] text-center select-none z-10">
        <div className="max-w-xl mx-auto px-8 flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-6 opacity-40">
          <p className="text-[12px] font-bold uppercase tracking-[0.2em]">©ATD26_SCIENCE-ART</p>
        </div>
      </footer>
    </div>
  );

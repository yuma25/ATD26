...
| `animationMixer`        | 内部アニメ制御     | GLB内のボーンアニメーション等の有効化（`animation-mixer`） |
...
現在の主要な作品設定の設計方針です。全ての作品は、ARマーカー（絵画）から飛び出す「圧倒的な存在感」を演出するため、適切なスケールとアニメーションが設定されています。
...
- **自然に寄り添う者たち (Common Blue)**: 
  ...
  - **内部アニメ**: `animation-mixer` により、3Dモデル固有の羽ばたきモーションを同期再生。
- **ちょっと不思議な海の冒険 (Shellcrab)**: 正面向きで静止しつつ、微細な浮遊感を持たせています。
- **海底の奥 (Antique Sword)**: 静かに回転しつつ、上下にゆっくりと浮遊します。
...
高品質な質感を維持しつつ、モデルの白飛びと沈み込みのバランスを最適化しています。

- **全体環境光 (Ambient)**: `0.7`（全体の視認性を確保する高めのベース光）
- **全体平行光 (Directional)**: `1.0`（エッジを強調し、立体感と質感を生み出す主光源）

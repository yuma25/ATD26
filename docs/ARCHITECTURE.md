...
3.  **L3: セルフホスト・アセット**: ARエンジン（MindAR, A-Frame）やデコーダー等の外部ライブラリを `public/scripts/` でホスト。CDN障害の影響を排除し、オフラインに近い安定性を確保。また、3Dモデルはブラウザキャッシュを有効化し、2回目以降のロードを高速化。
...
- **高品質レンダリング**: アンチエイリアス有効化、計算精度の向上 (`highp`)、およびスムージングフィルタにより、ジッター（微小振動）を抑えた滑らかな AR 体験を提供。
- **状態の即時反映**: パーティ人数入力時など、通信待ちの間に UI が再表示（フリッカー）しないよう、ローカル状態による先行的な UI 制御を実装。
